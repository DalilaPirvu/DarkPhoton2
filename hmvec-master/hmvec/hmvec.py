...
class HaloModel(Cosmology):
    ...
    def add_nfw_profile_new(self, name):
        ms200, rs200, cs200 = self.mrc200()

        lamb = self.p['lambda_blue']
        cs = lamb * cs200[...,None]
    ...
    def add_hod(self, name, corr="max",
        ...
        hod_params = ['hod_blue_siglogM', 'hod_Mmin_cendivh', 'hod_blue_alphasat', 
                      'hod_M1pr_satdivh', 'hod_M0_satgrowth', 'lambda_blue']
        ...
        mthresh = pparams['hod_Mmin_cendivh']#/self.h

        Ncs = blue_avg_Nc(logmhalo = np.log10(self.ms*self.h),
                          logm_minHOD = np.log10(mthresh),
                          siglogM = pparams['hod_blue_siglogM'])

        Nss = blue_avg_Ns(self.ms, Nc = Ncs,
                          alphasat = pparams['hod_blue_alphasat'],
                          m0 = pparams['hod_M0_satgrowth'],
                          m1pr = pparams['hod_M1pr_satdivh']/self.h)
    ...
    def get_Wg_from_data(self):
        dndz_data = np.transpose(np.loadtxt("./hmvec-master/data/blue.txt", dtype=float))
        dndz = np.interp(self.zs, dndz_data[0,:], dndz_data[1,:])
        N_gtot = np.trapz(dndz, self.zs)
        return dndz/N_gtot
...
def blue_avg_Nc(logmhalo, logm_minHOD, siglogM):
    """<Nc(m)>"""
    num = logmhalo - logm_minHOD
    return 0.5*(1. + erf(num/siglogM))

def blue_avg_Ns(ms, Nc, alphasat, m0, m1pr):
    num = ms - m0
